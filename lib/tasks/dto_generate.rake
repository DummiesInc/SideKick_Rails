...
namespace :dto do
  ...
  task generate: :environment do
    ...
    Rails.application.routes.routes.each do |route|
      ...
      # Determine DTOs for this action
      dto_info = if controller_class.const_defined?(:RETURN_DTO)
                   controller_class::RETURN_DTO[action.to_sym]
                 end
      next unless dto_info

      response_dto = dto_info[:response]
      request_dto  = dto_info[:request]

      # Responses schema: array for collection, single for :id
      response_schema = path.include?(':id') || %w[post put patch].include?(verb) ?
      ...
      if request_dto
        operation[:requestBody] = {
          required: true,
          content: {
            'application/json' => { schema: { '$ref' => "#/components/schemas/#{request_dto.name}" } }
          }
        }
      end
